...
int main(int argc, char** argv) {
    ...
    cl_int err;
    std::vector<int,aligned_allocator<int>> source_host_memory(ARRAY_TEST_DIM);
    std::vector<int,aligned_allocator<int>> hwOutput_memory(ARRAY_TEST_DIM);
    std::vector<int,aligned_allocator<int>> swOutput_memory(ARRAY_TEST_DIM);
    ...
    OCL_CHECK(err, err = /* Kernel Name, function name?? */.setArg(0, buffer_input));
    OCL_CHECK(err, err = /* Kernel Name, function name?? */.setArg(1, buffer_output));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(/* Kernel Name, function name?? */));
    q.finish();
